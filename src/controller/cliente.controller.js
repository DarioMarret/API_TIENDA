...
export const ShearClient = async (req, reply) => {
    try {
        ...
        console.log(data)
        console.log("\n")
        if(data.estado == "error" ) return NoCliente(cedula, reply) // crear funcion de no cliente 
        ...
        if(response.data.estado == "error"){
            ...
            reply.send([...info,{idfactura,idcliente,detalle:`Total a pagar ${total}`,total,"success":true, 
